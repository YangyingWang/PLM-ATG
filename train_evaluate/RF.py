...
def train(X_train, y_train):
    params = {
        ...
        'min_samples_split': randint(2, 10),  # 分割节点的最小样本数
        'min_samples_leaf': randint(1, 10),  # 叶节点的最小样本数
    }
    rf = RandomForestClassifier(max_features='sqrt', random_state=42)
    random_search = RandomizedSearchCV(rf, params, n_iter=150, cv=5, verbose=3, refit=True)
    ...
    joblib.dump(best_model, "D:/Major/AIProject/ATG/models/based_fasta/RF_AADP_fasta.pkl")
    with open("D:/Major/AIProject/ATG/performance/RF_results.txt", "a") as f:
        f.write(f"-------------------RF OF AADP_FASTA--------------------\n")
